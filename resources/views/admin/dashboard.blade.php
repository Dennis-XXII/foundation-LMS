<x-layout>
            ...
            @if(!$hasCollection && !$hasSingle)
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.courses.create') }}"
                       class="px-4 py-2 bg-blue-600 text-white rounded">Create Course</a>
                </div>
                    ...
                    <div class="border rounded-lg p-5">
                        <div class="flex items-start justify-between">
                            ...
                            <a href="{{ route('admin.courses.edit', $c) }}"
                               class="px-3 py-1.5 text-sm bg-blue-600 text-white rounded hover:bg-blue-700">Edit</a>
                        </div>

                        @if(isset($c->stats) && is_array($c->stats))
                            <div class="mt-3 text-sm text-gray-600">
                                Students: <span class="font-medium">{{ $c->stats['students'] ?? '0' }}</span> ·
                                Materials: <span class="font-medium">{{ $c->stats['materials'] ?? '0' }}</span> ·
                                Upload Links: <span class="font-medium">{{ $c->stats['assignments'] ?? '0' }}</span>
                            </div>
                        @endif

                        <div class="mt-4 flex flex-wrap gap-2">
                            {{-- Per-course ENROLLMENTS (add/remove students in this course) --}}
                            <a href="{{ route('admin.courses.students.index', $c) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    @foreach($courses as $c)
                        <div class="border rounded-lg p-5">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2">
                                {{-- Per‑course ENROLLMENTS (add/remove students in this course) --}}
                                <a href="{{ route('admin.courses.enrollments.index', $c) }}"
                                class="px-3 py-1.5 text-sm border rounded hover:bg-gray-50">
                                    Manage Enrollments
                                </a>
    ...
    <section class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="{{ route('admin.courses.index') }}" class="p-5 bg-white border rounded hover:bg-gray-50">
            <div class="text-sm text-gray-600">Browse</div>
            <div class="text-lg font-semibold">All Courses</div>
        </a>

        <a href="{{ route('admin.students.index') }}" class="p-5 bg-white border rounded hover:bg-gray-50">
            <div class="text-sm text-gray-600">Browse</div>
            <div class="text-lg font-semibold">All Students</div>
        </a>

        <a href="{{ route('admin.announcements.index') }}" class="p-5 bg-white border rounded hover:bg-gray-50">
            <div class="text-sm text-gray-600">Browse</div>
