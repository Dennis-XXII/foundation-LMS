<x-layout>
    ...
    <form class="bg-white border rounded-lg shadow p-6 space-y-4"
          method="POST" action="{{ route('admin.announcements.update', $announcement) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
        <input type="text" name="title" value="{{ old('title', $announcement->title) }}" class="w-full border rounded px-4 py-2" required>
        <textarea name="description" rows="3" class="w-full border rounded px-4 py-2">{{ old('description', $announcement->description) }}</textarea>
        ...
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div class="flex items-center gap-3">
                <label class="inline-flex items-center gap-2 mt-6">
                    <input type="checkbox" name="is_global" value="1" {{ old('is_global', $announcement->is_global) ? 'checked' : '' }}>
                    <span>Global</span>
                </label>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <form method="POST" action="{{ route('admin.announcements.destroy', $announcement) }}"
                ...
                @csrf @method('DELETE')
                <button class="px-4 py-2 bg-rose-600 text-white rounded">Delete</button>
            </form>
        </div>
    </form>
</x-layout>
