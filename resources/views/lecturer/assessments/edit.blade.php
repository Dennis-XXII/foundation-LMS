...
<x-layout>
    ...
    >
        ...
        <div class="mb-6 p-4 rounded-lg bg-gray-100 border border-gray-200">
            ...
            <p class="text-gray-700">
                Submitted At:
                {{ optional($submission->submitted_at)->format("d M Y, H:i") ?? "N/A" }}
            </p>
        </div>
        ...
        >
            ...
            @if ($submission->file_path)
                <div class="mb-4">
                    ...
                    <a
                        href="{{ route("lecturer.submissions.download", $submission) }}"
            ...
            <div>
                ...
                <div class="mt-1 flex items-center gap-2">
                    <input
                        ...
                        class="w-15 border rounded px-2 py-1 text-sm @error("score") border-red-500 @enderror"
                    />
            ...
            <div>
                ...
                <textarea
                    ...
                    class="mt-1 w-full border rounded px-3 py-2 text-sm @error("comment") border-red-500 @enderror"
                >
{{ old("comment", $assessment->comment ?? "") }}</textarea
                >
                @error("comment")
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input
                    ...
                    id="feedback_file"
                    name="feedback_file"
                    class="mt-1 w-1/2 block border border-gray-300 rounded-lg text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                />
                @error("feedback_file")
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
                ...
                @if ($isUpdating && $assessment->feedback_file_path)
                    <div class="mt-2 text-sm">
                        ...
                        <a
                            href="{{ route("lecturer.assessments.downloadFeedback", $assessment) }}"
                            class="text-green-700 hover:underline ml-2"
                        >
                            Download Feedback
                        </a>
                        <label class="text-xs text-red-600 ml-4">
                            <input
                                type="checkbox"
                                name="clear_feedback_file"
                                value="1"
                            />
                            Clear file on save
                        </label>
