  ...
  <nav class="mb-6 text-sm text-gray-600" aria-label="Breadcrumb">
    <ol class="list-reset flex">
      ...
      <li>
        ...
        <a href="{{ route('lecturer.courses.materials.index', [
            'course' => $course, 
            'type' => $type, 
            'level' => $level
          ...
          <table class="min-w-full text-sm border">
            ...
            <tbody>
              @forelse($materials as $m)
                <tr class="border-t">
                  <td class="px-3 py-2">
                    <a class="text-blue-600 underline" href="{{ route('lecturer.materials.show', $m) }}">{{ $m->title }}</a>
                  </td>
                  ...
                  <td class="px-3 py-2 space-x-3 whitespace-nowrap">
                    ...
                    @endif
                    <a class="text-gray-500 hover:underline" href="{{ route('lecturer.materials.edit', $m) }}">Quick Edit</a>
                    {{-- Delete form is modified to redirect back to this list view --}}
                    <form method="POST" action="{{ route('lecturer.materials.destroy', $m) }}" class="inline" onsubmit="return confirm('Delete this material?');">
                      @csrf @method('DELETE')
              ...
              @empty
                <tr><td class="px-3 py-4 text-gray-500" colspan="8">No materials found matching these filters.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
