...
<x-layout>
    <nav class="mb-2 text-sm text-gray-600 p-3" aria-label="Breadcrumb">
        <ol class="list-reset flex">
            ...
            <li class="text-black font-semibold">Add Material</li>
        </ol>
    </nav>
        ...
        <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow border">
            <div class="{{ $headerColor }} rounded p-4 mb-6">
                <h1 class="text-2xl font-semibold">
                    Add Materials —
                    {{ $type ? str($type)->replace("_", " ")->title() : "General" }}
                    Materials
                </h1>
                <h1 class="text-xl text-gray-600 font-thin pt-3">
                    Level - {{ $level ?? "—" }}
                </h1>
            </div>
            {{-- Validation errors --}}
            ...
            >
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="mt-1 w-full border border-gray-400 rounded px-3 py-2 min-h-[100px]"
                    >
{{ old("descriptions") }}</textarea
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        @if (request()->filled("type"))
                            ...
                            <div
                                class="mt-1 px-3 py-2 border border-gray-400 rounded bg-gray-50"
                            >
                        ...
                        @else
                            ...
                            <select
                                ...
                                required
                            >
                                @foreach (["lesson" => "Lesson Materials", "homework" => "Homeworks", "self_study" => "Self-study"] as $v => $label)
                                    <option
                                        value="{{ $v }}"
                                        @selected(old("type", request("type")) === $v)
                                    >
                                        {{ $label }}
                                    </option>
                                @endforeach
                    ...
                    <div class="flex items-center gap-2 md:col-start-1">
                        {{-- Moved to new line for grid flow --}}
                        <input
                            id="is_published"
                            name="is_published"
                            type="checkbox"
                ...
                <div class="border border-gray-400 rounded p-4 space-y-3">
                    <div>
                        ...
                        <input
                            type="file"
                            name="file"
                            class="block mb-1 w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-500 file:text-white hover:file:bg-blue-700 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none"
                        />
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ $backUrl }}" class="px-4 py-2 rounded border">
                        Back
                    </a>
                </div>
            </form>
        </div>
        <div class="max-w-3xl mx-auto mt-8 pt-4 border-t">
            <a
                href="{{
                    route("lecturer.courses.materials.index", [
                        "course" => $course,
