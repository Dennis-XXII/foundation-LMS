...
<x-layout>
  <nav class="mb-6 text-sm text-gray-600" aria-label="Breadcrumb">
    <ol class="list-reset flex">
      ...
      <li>
        <a href="{{ route('lecturer.courses.assignments.index', $assignment->course) }}" class="hover:underline">
          Upload Links
        </a>
        <span class="mx-2">/</span>
      </li>
  ...
  <div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow border">
    ...
    <div class="flex items-center justify-between pb-4 border-b">
      <div>
        ...
        <p class="text-lg text-gray-600">{{ $assignment->course->code }} — {{ $assignment->course->name }}</p>
      </div>
      <div class="flex gap-2">
        <a href="{{ route('lecturer.assignments.edit', $assignment) }}" class="px-4 py-2 rounded bg-blue-600 text-white text-sm font-medium">
          Edit
        </a>
        <form method="POST" action="{{ route('lecturer.assignments.destroy', $assignment) }}" onsubmit="return confirm('Are you sure you want to delete this assignment?');">
          @csrf
          @method('DELETE')
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="md:col-span-2 space-y-4">
        ...
        @if ($assignment->file_path)
          <div>
            ...
            <a href="{{ route('lecturer.assignments.download', $assignment) }}" class="mt-1 text-blue-600 hover:underline font-medium">
              Download Attached File
            </a>
          </div>
        @endif
        
        @if ($assignment->url) {{-- If you add URL later --}}
          <div>
            <label class="block text-sm font-medium text-gray-500">External Link</label>
            <a href="{{ $assignment->url }}" target="_blank" rel="noopener noreferrer" class="mt-1 text-blue-600 hover:underline font-medium">
              {{ $assignment->url }}
            </a>
          </div>
        @endif
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <p class="mt-1 text-gray-900">{{ optional($assignment->due_at)->format('M d, Y') ?? '—' }}</p>
        </div>
      </div>
    </div>
                        ...
                        <tr class="border-t">
                            ...
                            <td class="px-3 py-2">
                                @if($s->file_path)
                                    ...
                                    <a class="text-blue-600 underline" href="#">Download Submission</a>
                                @else
    ...
    <div class="mt-8 pt-4 border-t">
        <a href="{{ route('lecturer.courses.assignments.index', $assignment->course) }}" class="px-4 py-2 rounded border text-sm">
