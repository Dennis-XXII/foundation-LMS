...
<x-layout>
  <nav class="mb-6 text-sm text-gray-600" aria-label="Breadcrumb">
    <ol class="list-reset flex">
      ...
      <li>
        <a href="{{ route('lecturer.courses.assignments.index', $course) }}{{ request('level') ? '?level='.request('level') : '' }}" class="hover:underline">
          Upload Links
        </a>
        <span class="mx-2">/</span>
      </li>
    ...
    <form class="mt-6 space-y-4"
      ...
      <div>
        ...
        <textarea name="instruction"
                  ...
                  class="mt-1 w-full border rounded px-3 py-2 min-h-[100px]">{{ old('instruction') }}</textarea>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select name="week" class="mt-1 w-full border rounded px-3 py-2">
            ...
            @foreach(range(1, 8) as $w)
              <option value="{{ $w }}" @selected(old('week', request('week')) == $w)>Week {{ $w }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div>
          ...
          <select name="day" class="mt-1 w-full border rounded px-3 py-2">
            ...
            @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'REVIEW'] as $dayName)
              <option value="{{ $dayName }}" @selected(old('day', request('day')) == $dayName)>{{ $dayName }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div>
          ...
          <input type="date" name="due_at" value="{{ old('due_at') }}" class="mt-1 w-full border rounded px-3 py-2">
        </div>
        ...
        <div class="flex items-center gap-2 md:col-start-1">
          ...
          <label for="is_published" class="text-sm">Published (Active)</label>
        </div>
      </div>
      ...
      <div class="border rounded p-4 space-y-3">
        <div>
          <label class="block text-xs text-gray-600">Attachment (optional)</label>
      ...
      <div class="flex gap-3">
        <button class="px-4 py-2 rounded bg-black text-white">Create Link</button>
        @php
        ...
        @endphp
        <a href="{{ route('lecturer.courses.assignments.index', $course) . ($backParams ? '?'.$backParams : '') }}" class="px-4 py-2 rounded border">Back</a>
      </div>
    </form>
  </div>
</x-layout>