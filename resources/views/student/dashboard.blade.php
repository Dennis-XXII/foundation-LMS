            ...
            <div class="p-6 flex gap-6">
                ...
                <aside class="w-40 space-y-4">
                    ...
                    <button
                        class="w-full bg-gray-100 border border-gray-300 rounded p-4 text-sm hover:bg-gray-50"
                        onclick="
                            location.href = 'https://rsuip.org/about-us/campus/'
                        "
                    >
                        Maps
                    </button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 flex-1">
                    @foreach ($tiles as $t)
                        @if ($t["level"] <= $student_level && $student_level !== null)
                            {{-- Show only tiles at or below student's enrolled level --}}
                            @php
                            ...
                            @else
                                <div
                                    class="block rounded-lg p-5 border border-gray-300 opacity-60 cursor-not-allowed {{ $t["color"] }}"
                                    aria-disabled="true"
                                    title="Enroll in a course to access this"
                                >
                                    <div class="text-xs text-gray-600 mb-1">
                                        LEVEL {{ $t["level"] }}
                                    </div>
