    ...
    <div class="bg-white rounded-xl shadow p-6 space-y-3">
        <div class="text-sm text-gray-600">
            ...
            @if($submission->graded_at)
                <p>Graded: {{ $submission->graded_at->format('M d, Y H:i') }}</p>
            @endif
        </div>
        ...
        @if($submission->assessment)
            <div class="mt-2 p-4 bg-green-50 rounded-lg">
                <p class="font-medium">Score: {{ $submission->assessment->score }}</p>
                @if($submission->assessment->comment)
                    <p class="text-gray-700 mt-1">{{ $submission->assessment->comment }}</p>
                @endif
            </div>
        @else
        ...
        <div class="pt-2">
            @if(!$submission->graded_at)
                <a href="{{ route('student.assignments.submissions.edit', [$assignment, $submission]) }}" class="text-blue-600 hover:underline">Edit submission</a>
            @endif
            <span class="mx-2 text-gray-400">•</span>
            <a href="{{ route('student.assignments.index', $assignment->course) }}" class="text-gray-600 hover:underline">Back to assignments</a>
        </div>
    </div>
</div>
</x-layout>
